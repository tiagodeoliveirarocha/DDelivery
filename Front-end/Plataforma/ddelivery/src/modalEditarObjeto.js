...
function ModalEditarObjeto(props) {

  const [ordemObjeto, setOrdemobjeto] = useState([])
  const getOrdemobjeto = async () => {
    const response = await axios.get(url+"/ordens");
    setOrdemobjeto(response.data)
  }
  useEffect(() => {
    getOrdemobjeto();
  }, []);
  ...
  useEffect(() => {
    ...
    const [destinatarionome, setDestinatarionome] = useState(props.nomeobjeto);
    const [destinatariosobrenome, setDestinatariosobrenome] = useState(props.sobrenomeobjeto);
    const [remetente, setRemetente] = useState(props.remetenteobjeto);
    const [cidade, setCidade] = useState(props.objetocidade);
    const [estado, setEstado] = useState(props.objetoestado);
    const [tipo, setTipo] = useState(props.objetotipo);
    const [status, setStatus] = useState(props.objetostatus);
    const [cep, setCep] = useState(props.objetocep);
    const [bairro, setBairro] = useState("");
    const [observacao, setObservacao] = useState(props.objetoobservacao);
    const [peso, setPeso] = useState(props.objetopeso);
    const [enderecodestinatario, setEnderecodestinatario] = useState(props.objetodestinatario);
    const [prioridade, setPrioridade] = useState(props.objetoprioridade);
    const [ordem, setOrdem] = useState(props.ordemObjetoEditar);
    const id = props.codigoobjeto;
  ...
  const handleSubmit = (e) => {
    ...
    axios.put('http://localhost:3001/api/objeto', { 
      destinatarionome,
      destinatariosobrenome,
      remetente,
      cidade,
    ...
    <button className="btn btn-link btn-sm" onClick={handleShow}><BsPencilSquare /></button>
        ...
        <Modal show={show} onHide={handleClose} onSubmit={handleSubmit} size="xl" validated={validated}>
          ...
          <Modal.Header closeButton>
            <Modal.Title>Editar objeto #{id}</Modal.Title>
          </Modal.Header>
          <Modal.Body>
...
<div className="col">
      <label for="inputOrdem" className="form-label">Ordem de Entrega</label>

<select id="inputOrdem" className="form-control" value={ordem} onChange={(e) => setOrdem(e.target.value)}>
{ordemObjeto.map(objetoOrdem => <option >{objetoOrdem.id} </option>)}
</select>
...
<button className="btn btn-primary" onClick={handleSubmit}>Salvar</button>
  ...
  export default ModalEditarObjeto;